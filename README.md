...
2. Set up for GPU acceleration (optional). Some of the computations are intensive and benefit from GPU acceleration. If you have access to a GPU and want to use it, make sure you have the GPU version of pytorch installed. If the CPU version is installed, simply remove it with ```pip uninstall torch```. Then, run ```pip install torch==2.0.1+cu118 -f https://download.pytorch.org/whl/torch_stable.html```. This will install pytorch with CUDA 11.8. If this fails, you might need to check your NVIDIA driver for the GPU and that the toolkit CUDA 11.8 is installed. Note that if you're running the Google Colab version above, you can use the GPU provided by Google. See notebook for details.
