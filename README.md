...
2. Set up for GPU acceleration (optional). Some of the computations are intensive and benefit from GPU acceleration. If you have access to a GPU and want to use it, make sure you have the GPU version of pytorch installed. If the CPU version is installed, simply remove it with ```pip uninstall torch```. Then, run ```pip install --index-url https://download.pytorch.org/whl/cu118```. This will install pytorch with CUDA 11.8. If this fails, you might need to check your NVIDIA driver for the GPU and that the toolkit CUDA 11.8 is installed.

4. Install TTGC by pip installing or git clone / download.
